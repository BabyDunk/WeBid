...
while ($row = $db->fetch()) {
    ...
    $template->assign_block_vars('featured', array(
            ...
            'IMAGE' => (!empty($row['pict_url'])) ? 'getthumb.php?w=' . $system->SETTINGS['thumb_show'] . '&amp;fromfile=' . UPLOAD_FOLDER . $row['id'] . '/' . $row['pict_url'] : 'getthumb.php?w=' . $system->SETTINGS['thumb_show'] . '&amp;fromfile=' . get_lang_img('nopicture.png'),
